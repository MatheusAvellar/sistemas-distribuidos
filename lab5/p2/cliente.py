...
def main():
  ...
  try:
    ...
    print("Olá professora! Infelizmente esse programa ainda não suporta\nmúltiplos clientes paralelos.")
    print("Por sorte, isso não é um requisito na especificação do trabalho!")
    print("Nota mental: usar RPC da próxima vez\n\n\n")
    raise
  cli_sock.listen(5)
...
def acceptConn(new_sock,addr):
  while True:
    ...
    if not data:
      new_sock.close()
      break
    # Resposta é do tipo "[ID da busca],[ID do nó] [valor]
    # Caso não haja valor, [valor] é igual a "_null"
...
def sendTo(from_node_id,message):
  node_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # Desculpa prof eu sei que esse programa é 100% gambiarra mas é que
    # eu realmente fiquei sem tempo :'(  juro que não foi minha intenção
